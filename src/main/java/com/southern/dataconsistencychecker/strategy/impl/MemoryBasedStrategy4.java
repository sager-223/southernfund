...
//                                    // 记录详细日志
//                                    log.setSourceFieldKey(sourceField);
//                                    log.setSourceFieldValue(sourceValue);
//                                    log.setTargetFieldKey(targetField);
...
//                                compareDetailLogMapper.updateRepairSQL(log); // 假设有此Mapper方法
//
...
//                            Map<String, String> sourceRow = getRowData(sourceRs, sourceUniqueKeys, sourceCompareFields);
//
//                            String insertSourceSQL = generateInsertSQL(sourceTable, sourceRow);
//                            String deleteTargetSQL = generateDeleteSQL(targetTable, sourceKey, targetUniqueKeys);
//
...
//                            Map<String, String> targetRow = getRowData(targetRs, targetUniqueKeys, targetCompareFields);
//
//                            String deleteSourceSQL = generateDeleteSQL(sourceTable, targetKey, sourceUniqueKeys);
//                            String insertTargetSQL = generateInsertSQL(targetTable, targetRow);
//
...
//         * 生成 INSERT SQL 语句
//         */
//        private String generateInsertSQL(String tableName, Map<String, String> rowData) {
//            StringBuilder sb = new StringBuilder();
//            sb.append("INSERT INTO ").append(tableName).append(" (");
//            sb.append(String.join(", ", rowData.keySet()));
//            sb.append(") VALUES (");
...
//         * 生成 DELETE SQL 语句
//         */
//        private String generateDeleteSQL(String tableName, String compositeKey, String[] uniqueKeys) {
//            StringBuilder sb = new StringBuilder();
//            sb.append("DELETE FROM ").append(tableName).append(" WHERE ");
//            List<String> conditions = new ArrayList<>();
//            String[] keyParts = compositeKey.split("_");
//            for (int i = 0; i < uniqueKeys.length; i++) {
//                String key = uniqueKeys[i].trim();
...
//                                String targetValue = targetRow.get(targetField);
//                                if (!Objects.equals(sourceValue, targetValue)) {
//                                    differingFields.add(sourceField);
//                                    // 收集需要更新的字段
//                                    fieldsToUpdateSource.put(sourceField, targetValue);
//                                    fieldsToUpdateTarget.put(targetField, sourceValue);
//                                }
...
//
//                                // 设置详细字段信息（可以选择性设置）
//                                log.setSourceFieldKey(String.join(", ", differingFields));
//                                log.setSourceFieldValue(
//                                        String.join(", ", differingFields.stream()
//                                                .map(field -> field + "='" + targetRow.get(field) + "'")
//                                                .toArray(String[]::new))
//                                );
//                                log.setTargetFieldKey(String.join(", ", differingFields));
//                                log.setTargetFieldValue(
//                                        String.join(", ", differingFields.stream()
//                                                .map(field -> field + "='" + sourceRow.get(field) + "'")
//                                                .toArray(String[]::new))
//                                );
//
//                                // 设置修复SQL
...
//
//                            String insertSourceSQL = generateInsertSQL(sourceTable, sourceRow);
//                            String deleteTargetSQL = generateDeleteSQL(targetTable, sourceRow, targetUniqueKeys);
//
//                            // 设置修复SQL
//                            log.setRepairSource(insertSourceSQL);
//                            log.setRepairTarget(deleteTargetSQL);
...
//                            Map<String, String> targetRow = getRowData(targetRs, targetUniqueKeys, targetCompareFields);
//
//                            String deleteSourceSQL = generateDeleteSQL(sourceTable, targetRow, sourceUniqueKeys);
//                            String insertTargetSQL = generateInsertSQL(targetTable, targetRow);
//
//                            // 设置修复SQL
//                            log.setRepairSource(deleteSourceSQL);
...
//                                // **修正部分：正确记录源和目标字段的原始值**
//                                log.setSourceFieldKey(String.join(", ", differingFields));
//                                log.setSourceFieldValue(String.join(", ", differingFields.stream()
//                                        .map(field -> sourceRow.get(field))
//                                        .toArray(String[]::new))
//                                );
//                                log.setTargetFieldKey(String.join(", ", differingFields));
//                                log.setTargetFieldValue(String.join(", ", differingFields.stream()
...
public class MemoryBasedStrategy4 implements ConsistencyCheckStrategy {
    ...
    private class ShardCompareTask implements Callable<ShardCompareResult> {
                ...
                while (sourceHasNext || targetHasNext) {
                    if (sourceHasNext && targetHasNext) {
                        ...
                        if (comparison == 0) {
                            ...
                            String insertTargetSQL = generateInsertSQL(targetTable, sourceRow);
                            ...
                            String insertSourceSQL = generateInsertSQL(sourceTable, targetRow);
        ...
        private String generateDeleteSQL(String tableName, Map<String, String> rowData, String[] uniqueKeys) {
            ...
            if (hasNull && conditions.size() == uniqueKeys.length) {
                // 此处可以选择记录日志或抛出异常，根据具体需求调整
